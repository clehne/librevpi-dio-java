...
//		System.out.println("Load from JAR: " + fileName);
	...
	/**
	 *	sets digtal channel value for kunbus 
	 * @param _channelAlias, e.g. O_0
	 * @param value  true or false  
	 * @throws IOException in case of any error
	 */
	public void setDigital(String _channelAlias, boolean _value) throws IOException {
		int val = _setValueDIO(_fd, _channelAlias, (_value?1:0));
		if(val < 0) {
	...
	/**
	 *	reads in digital channel value for kunbus
	 * @param _channelAlias, e.g. I_0
	 * @return true or false 
	 * @throws IOException in case of any error
	 */
	public boolean getDigital(String _channelAlias) throws IOException {
		int val = _getValueDIO(_fd, _channelAlias);
		if(val < 0) {
        ...
        /**
	 *      sets byte value for kunbus channel alias
	 * @param _channelAlias, e.g. RevPiLED 
	 * @param value  true or false
	 * @throws IOException in case of any error
	 */
	 public void setValue(String _channelAlias, int _value) throws IOException {
	        int val = _setValueDIO(_fd, _channelAlias, _value);
	        if(val < 0) {
	               throw new IOException("Error setting Data OUT " + _channelAlias + " retCode " + val);
	        }
        }
        ...
        /**
	 *      gets byte value for kunbus channel alias
	 * @param _channelAlias, e.g. RevPiLED 
	 * @return true or false
	 * @throws IOException in case of any error
	 */
        public int getValue(String _channelAlias) throws IOException {
               int val = _getValueDIO(_fd, _channelAlias);
               if(val < 0) {
                       throw new IOException("Error reading Data OUT " + _channelAlias + " retCode " + val);
               }
	...
	private static native void _closeDIO(final int fd) throws IOException;

	private static native int _setValueDIO(final int fd, String channelAlias, int value);
	private static native int _getValueDIO(final int fd, String channelAlias);
}